...
class ElectrusFindData:
    ...
    def execute(self) -> DatabaseActionResult:
        try:
            ...
            return DatabaseActionResult(success = True, matched_count = len(paginated_data), data = paginated_data, inserted_ids = [_dict['_id'] for _dict in paginated_data])
        except Exception as e:
        ...
        try:
            ...
            for item in collection_data:
                if operator_checker.evaluate(item, filter_query):
                    ...
                    return DatabaseActionResult(success = True, matched_count = 1, data = item, inserted_ids = item.get("_id"))
